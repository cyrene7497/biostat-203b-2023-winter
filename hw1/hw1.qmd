...
ls ~/mimic/icu
```
All the data in these folders are in `csv.gz` format, rather than `.csv`. The `csv.gz` is a `.csv` file compressed with the `gzip` utility for storage. Since MIMIC-VI encompasses a large amount of data, it's preferable to store data in this compressed version.

The `hosp` module had data obtained from the hospital-wide electronic health record. This information includes patient and admission information, laboratory results, and billed diagnoses. The `icu` module provides data for emergency department patients collected while they're in the emergency department. This information includes why they were admitted, their triage assessment results, vital signs, and any medicine reconcilliation. The `core` module has been removed from the current version of MIMIC-IV so there isn't much documentation on it on their website. The core module had information on admissions, patients, and transfers, which are now included in the `hosp` module. 
...
```
The output of `ls -l` is the detailed contents of the directory. In this case, the loop calls all the files ending in `.gz` within the `core` module within the `mimic` folder. Then `ls -l` outputs the permissions, owner, size and date and time last modified of each file ending in `.gz` within that folder. 
...
```
The `admissions` file has 523,741 lines, the `patients` file has 382,279 lines, and the `transfers` file has 2,189,536 lines. 
...
```
There are 523,741 rows in this data file. It includes 256,878 unique patients identified by `subject_id` which is the first columm in the data file, seen in the first few lines.
...
```
The possible values and counts of the variable `admission_type` are 7,254 entries of `Ambulatory Observation`, 21,581 entries of `Direct Emer.`, 19,991 entries of `Direct Observation`, 72,072 entries of `Elective`, 100,445 entries of `EU Observation`, 157,896 entries of `EW Emer.`, 55,497 entries of `Observational Admit`, 41,074 entries of `Surgical Save Day Admission`, and 47,930 of `Urgent`. 
```{bash}
gzcat ~/mimic/core/admissions.csv.gz | tail -n+2 | awk -F, '{ print $7}' | sort | uniq -c
```
The values and counts of the variable `admission_location` are 60,435 empty entries, 191 entries of `Ambulatory Surgery Transfer`, 10,670 entries of `Clinical Referral`, 245,744 entries of `Emergency Room`, 379 entries of `Information Not Available`, 4,467 entries of `Internal Transfer to or from Psych`, 6,067 entries of `PACU`, 127,494 entries of `Physician Referral`, 8,449 entries of `Procedure Site`, 39,121 entries of `Transfer From Hospital`, 4,063 entries of `Transfer From Skilled Nursing Facility`, and 16,660 entries of `Walk-in/Self Referral`. 
```{bash}
gzcat ~/mimic/core/admissions.csv.gz | tail -n+2 | awk -F, '{ print $9}' | sort | uniq -c
```
The values and counts of the variable `insurance` are 50,850 entries of `Medicaid`, 171,360 entries of `Medicare`, and 301,530 entries of `Other`. 

```{bash}
gzcat ~/mimic/core/admissions.csv.gz | tail -n+2 | awk -F, '{ print $12}' | sort | uniq -c
```
The values and counts of the variable `ethnicity` are 
